...
func (s *SnapshotTester) takeSnapshot(snapshotName string) (string, error) {
...
func (s *SnapshotTester) Assert(t testingT) {
	t.Helper()
	newSnap, err := s.takeSnapshot(t.Name())
	if err != nil {
		t.Error(err)
		return
	}
...
func (s *SnapshotTester) compare(new, old string) (string, bool) {
	if new == old {
		return "", false
	}
